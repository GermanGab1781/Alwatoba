...
export default function Navbar(){
  ...
  return (
    <motion.div className={navButton 
      ?"custom-BgColorMain custom-Color4 grid grid-rows-1 grid-cols-9 content-center fixed z-50 w-screen h-10 sm:p-8 sm:pt-8 p-10 transition-all ease-in-out duration-1000 delay-75" 
    ...
    animate={{opacity:1}} exit={{opacity:0}}>
      ...
      {/* img Brand */}
      <NavLink onClick={()=>setNavButton(true)} className="sm:col-start-1 sm:col-end-2 sm:ml-2 ml-5 row-start-0 row-end-1 col-start-1 col-span-3 text-center hover:scale-110 transition-all delay-75" to="/Alwatoba"><span className="sm:h-full sm:w-full w-1/2 h-full font-bold text-6xl m-auto font-mathlete sm:text-red-800 md:text-amber-400 lg:text-lime-500 xl:text-sky-500 2xl:text-violet-700" >ALWATOBA</span></NavLink>
      {/* Nav Opener */}      
        <button onClick={()=>setNavButton(!navButton)} className="row-start-0 sm:col-start-9 col-start-8 hover:text-slate-50 sm:ml-10 ml-4  transition-all delay-75"><FaBars className="sm:w-10 sm:h-12 h-10 w-8"/></button>      
      {/* Nav Items */}
      <div className={navButton 
        ...
        <NavLink className="hover:text-slate-50 transition-all delay-75" to="/Alwatoba" onClick={()=>clickEv()}>Inicio</NavLink>
        ...
        <span className="lg:mt-5 md:mt-3 sm:mt-1 mt-0.5 cursor-pointer hover:text-slate-100 transition-colors delay-75" onClick={()=>setNavButton(!navButton)}>Volver</span>
      </div>
    </motion.div>
  )
}
