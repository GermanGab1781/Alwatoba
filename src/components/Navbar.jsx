...
export default function Navbar(){
  ...
  return (
    <motion.div className={navButton 
      ?"flex font-comfortaa flex-col bg-slate-50 fixed z-40 w-screen md:h-10 h-10 md:py-10 py-6 place-content-center transition-all ease-in-out duration-1000 delay-75" 
    ...
    animate={{opacity:1}} exit={{opacity:0}}>
      <div className='flex flex-row place-content-between md:px-10 px-5 absolute top-2 w-screen'>        
        {/* img Brand */}
        <NavLink onClick={()=>setNavButton(true)} className=" text-center transition-all delay-75" to="/Alwatoba"><span className="sm:h-full sm:w-full w-1/2 h-full font-bold sm:text-6xl text-3xl m-auto" >Alwatoba</span></NavLink>
        {/* Nav Opener */}      
      ...
      <div className={navButton 
        ...
        <>
        <NavLink className="text-center cursor-pointer md:text-5xl text-4xl custom-Color4 hover:font-bold transition-all delay-75" to="/Alwatoba" onClick={()=>setNavButton(!navButton)}>Inicio</NavLink>
        ...
        <span className="text-center cursor-pointer md:text-5xl text-4xl custom-Color4 hover:font-bold transition-all delay-75 mt-10" onClick={()=>setNavButton(!navButton)}>Volver</span>
        </>
