  ...
  return (
    <motion.div className="bg-amber-300 pt-16 pb-10 grid grid-cols-8 text-center" initial={{opacity:1}} animate={{opacity:1}} exit={{opacity:1}}>
        ...
        <div className={menuButton 
          ...
          <button className="sm:invisible visible absolute bg-amber-200 -right-6 bottom-1/2  rounded-tr-full rounded-br-full pr-3 pl-1" onClick={()=>setMenuButton(!menuButton)}>F<br/>I<br/>L<br/>T<br/>R<br/>A<br/>R</button>
        </div>
        {/* Catalogo */}
        <div className="sm:bg-amber-400 bg-amber-400 row-start-2 sm:col-start-2 sm:col-span-7 col-start-1 col-end-9 min-h-screen max-h-fit pl-2">
          <Outlet/>
        </div>
    </motion.div>
  )
